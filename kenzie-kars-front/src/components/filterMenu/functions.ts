...
export const handleFilterConditions = async (
...
export const handleFilterElements = async (
  filterConditions: iFilterConditions
) => {
  if (filterConditions.brand) {
    try {
      const response = await fipeApi.get(
        `/cars?brand=${filterConditions.brand.toLocaleLowerCase()}`
      );
      const cars = response.data.map((element: any) => element.name);
      const firstWordsArray: Array<string> = cars?.map(
...
export const handleFilterOptionsYearAndFuel = async (
  filterConditions: iFilterConditions
) => {
  if (filterConditions.brand) {
    try {
      ...
      const availableYears: Array<string> = data.map(
        (element: any) => element.year
      );
      const availableFuel: Array<string> = data.map(
        (element: any) => element.fuel
      );

      return {
        availableYears: [...new Set(availableYears)],
        availableFuel: [...new Set(availableFuel)],
      };
    } catch (error) {
      console.error("Error fetching models", error);
    }
